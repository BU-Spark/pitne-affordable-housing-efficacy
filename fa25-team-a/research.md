...
*How far is an applicant willing to move from their current home? How does the applicant's geographic range compare between different demographic groups?*
* Depending on which organization is responsible, the application can be very different (e.g., requiring extra documentation or being longer).
...
*How far is an applicant willing to move from their current home?*
* To analyze how far an applicant is willing to move from their current home, we can compare the zip codes of the house applied for and the current home address (refer to columns **Application Property** and **Current Residence** in the CHAPA Chapter 40B Application Data dataset).
* Using **Applicant ID** as a primary key, conduct separate spatial analysis for standard and 55+ applicants, with the PIT-NE Summer team's cleaned age-restricted datasets.
* Distance between the two addresses can be measured by commute time.
  * Commute time could also provide insight into the accessibility of a place. For example, some applicants may be more willing to move to a place with greater public transit accessibility.
    
*How does the applicant's geographic range compare between different demographic groups?*
* Use the PIT-NE team's demographic grouping to compare each demographic group's average geographic movement (based on commute time).
...
* Possible model(s): Poisson or negative binomial regression model
  * The feature variable price is used to model the count of applications.
...
* Similarly, we can compare proportions of different demographic applications (such as different income levels) to towns with the highest price and those with the lowest price.
* Possible model(s): Multinomial regression
  * The model would estimate how price changes affect the probability that an applicant belongs to each demographic group.
  * This could be done for different types of demographic groups (such as income, race, and household size).
